...
function HPC:ApplySpecProfileLogic()
    if not self.db.global.useSpecProfile then
        local defaultProfile = UnitName("player") .. " - " .. GetRealmName()
        if self.db:GetCurrentProfile() ~= defaultProfile then
            self.db:SetProfile(defaultProfile)
            self:ApplySettings()
        end
        return
    end
    ...
    local expectedProfile = UnitName("player") .. " - " .. specName
    if self.db:GetCurrentProfile() ~= expectedProfile then
        self.db:SetProfile(expectedProfile)
        self:ApplySettings()
    end
end

function HPC:PLAYER_SPECIALIZATION_CHANGED(event, unit)
    if unit == "player" then
        self:ApplySpecProfileLogic()
    end
end
...
function HPC:HandleBlizzardBar(isInitialLoad)
    if self.db.profile.hideBlizzBar then
        -- Midnight 12.0 Modern Class Power Bar approach
        -- WARNING: Calling :Hide() or :UnregisterAllEvents() on this secure frame heavily taints our addon!
        -- Tainted addons are completely blocked from reading aura spellIDs in combat natively.
        -- Thus, we rely solely on visually setting alpha to 0 to bypass the security panic.
        if PlayerFrame and PlayerFrame.classPowerBar then
            PlayerFrame.classPowerBar:SetAlpha(0)
            
            -- Keep it invisible if Blizzard tries to fade it back in
            if not self.blizzBarHooked then
                hooksecurefunc(PlayerFrame.classPowerBar, "SetAlpha", function(bar, alpha)
                    if HPC.db.profile.hideBlizzBar and alpha > 0 then
                        bar:SetAlpha(0)
                    end
                end)
                self.blizzBarHooked = true
            end
        end
        -- Legacy fallback
        ...
        args = {
            general = {
                ...
                args = {
                    ...
                    useSpecProfile = {
                        name = "Use Per-Specialization Settings",
                        desc = "When enabled, creates a separate profile for each specialization (e.g. Holy vs Ret) so you can position them differently.",
                        width = "full",
                        type = "toggle", order = 1.1,
                        get = function() return self.db.global.useSpecProfile end,
                        set = function(_, val) 
                            self.db.global.useSpecProfile = val
                            self:ApplySpecProfileLogic() 
                        end,
                    },
                    explanation = {
